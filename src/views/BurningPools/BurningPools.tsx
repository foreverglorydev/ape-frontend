...
import { Heading, Text, Card, Checkbox, ArrowDropDownIcon, useModal } from '@apeswapfinance/uikit'
...
import { useFarms, usePriceBnbBusd, useBurningPools, useStatsOverall } from 'state/hooks'
import { BurningPool } from 'state/types'
import { QuoteToken, PoolCategory } from 'config/constants/types'
import Page from 'components/layout/Page'
import RiskDisclaimer from './Disclaimer'
...
export interface BurningPoolWithStakeValue extends BurningPool {
  apr?: BigNumber
  staked?: BigNumber
  addStakedUrl?: string
...
const Header = styled.div`
  ...
  background-image: ${({ theme }) =>
    theme.isDark ? 'url(/images/burning-pool-background-night.svg)' : 'url(/images/burning-pool-background-day.svg)'};
...
const Pools: React.FC = () => {
  ...
  const [hasAcceptedRisk, setHasAcceptedRisk] = usePersistState(false, {
    localStorageKey: 'burning_pools_accepted_risk',
  })
  const { account } = useWeb3React()
  const { pathname } = useLocation()
  const size: Size = useWindowSize()
  const farms = useFarms()
  const allBurningPools = useBurningPools(account)
  ...
  const poolsWithApy = allBurningPools.map((pool) => {
    ...
    const stats = statsOverall?.incentivizedPools?.find((x) => x.id === 21)
  ...
  const sortPools = (poolsToSort: BurningPoolWithStakeValue[]) => {
    switch (sortOption) {
      case 'apr':
        // Ternary is needed to prevent pools without APR (like MIX) getting top spot
        return orderBy(poolsToSort, (pool: BurningPoolWithStakeValue) => pool.apr.toNumber(), sortDirection)
      case 'earned':
        return orderBy(
          poolsToSort,
          (pool: BurningPoolWithStakeValue) => {
      ...
      case 'totalStaked':
        return orderBy(
          poolsToSort,
          (pool: BurningPoolWithStakeValue) => getBalanceNumber(pool.totalStaked) * pool.stakedTokenPrice,
          sortDirection,
        )
      default:
        return orderBy(poolsToSort, (pool: BurningPoolWithStakeValue) => pool.sortOrder, 'asc')
    }
  }
