...
const cleanJunglePoolData = (poolIds: number[], chunkedPools: any[], tokenPrices: TokenPrices[], chainId: number) => {
  const data = chunkedPools.map((chunk, index) => {
    const poolConfig = junglePools.find((pool) => pool.sousId === poolIds[index])
    const [startBlock, endBlock, totalStaked] = chunk
    const totalStakedFormatted = new BigNumber(totalStaked).toJSON()
    const [stakingToken, rewardToken, apr] = fetchJunglePoolTokenStatsAndApr(
      poolConfig,
      tokenPrices,
...
const fetchJunglePoolTokenStatsAndApr = (
  pool: PoolConfig,
...
export default cleanJunglePoolData
